  ...
  await db.$transaction([
    ...
    db.message.create({
      data: {
        orderId: order.id,
        senderId: admin!.id,
        content: `🛡️ ROBLOX BOUTIK — Commande validée par l'administration. Le vendeur a reçu ${netAmount} FCFA (montant net après commission de 20%, soit ${commission} FCFA conservés par la plateforme). Merci pour votre achat !`,
        isAuto: true,
      },
    }),
  ]);
